...
const comboboxState = ref(createComboboxState({ open: false }))
comboboxState.value = setComboboxFilter(comboboxState.value, query.value)
...
watch(
  ...
  (open) => {
    comboboxState.value = setComboboxOpen(comboboxState.value, open)
    if (open) {
      nextTick(() => {
        updatePosition()
...
watch(
  () => query.value,
  (value) => {
    comboboxState.value = setComboboxFilter(comboboxState.value, value)
  },
  { immediate: true },
)
...
function moveFocus(delta: number) {
  comboboxState.value = moveComboboxFocus({ state: comboboxState.value, context: context.value, delta })
...
function commitSelection(index: number) {
  ...
  if (!option) return
  comboboxState.value = activateComboboxIndex({ state: comboboxState.value, context: context.value, index })
...
<template>
  <article class="combobox-card" ref="triggerRef">
    ...
    <div class="combobox-field">
      ...
      <div class="combobox-input-shell">
        <input
...
.combobox-card {
  ...
  background: radial-gradient(circle at top, rgba(99, 102, 241, 0.12), transparent 65%), var(--surface-card);
  display: flex;
