...
- [x] Финальная оценка пункта: `9.5`.
- Комментарий по закрытию: `2026-02-07` - стабилизирована idempotency repeated refresh: в `tableViewportVirtualization.ts` row-pool version больше не инкрементируется на неизменном окне, `onRows` теперь эмитится только при реальном изменении snapshot/геометрии; в `tableViewportController.ts` добавлен dedupe для `onColumns` и `onScrollSync` (signature-based), чтобы forced refresh без state-delta не генерировал повторные imperative events. Для resize-storm в `tableViewportScrollIo.ts` добавлен guard: heavy update от ResizeObserver запускается только при изменении layout-метрик (`captureLayoutMetrics` возвращает `boolean`). Добавлен regression suite `packages/datagrid-core/src/viewport/__tests__/scrollResizeDeterminism.contract.spec.ts` (idempotent forced refresh + deterministic replay resize/scroll storm), и fail-fast lock через `packages/datagrid-core/package.json` (`test:regressions`) + root `package.json` (`test:datagrid:regressions` включён в `quality:lock:datagrid`).
